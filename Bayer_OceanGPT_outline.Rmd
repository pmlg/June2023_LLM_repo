...
explain tokenisation, explain cost per token.

explain training process: not trained on truth, but on producing the most likely next piece of text given half a piece of text (and that half piece can be a question).

more an art than a craft - we rarely can predict why something works. lots of 'best practises', no standards. (Yang Harnessing the power...)paper)
...
Have 37,000 eDNA papers to summarise or interrogate. Might build a summariser that goes to openGPT *once* for a chunk of text, then just talk to the local summariser. But my act of deciding what to summarise will skew what users can ask.
...
human-like behaviour in rocks. Talk a bit about stochastic parrots (Emily Bender paper/Ted Chiang 'averaging' article). ChatGPT *probably* (IMHO) cannot generalise/find novel ideas. I've played around a lot with trying to get OceanGPT to find novel patterns in our data, usually terrible at this. Only good if I get an ecologist or a fisheries scientist to ask their questions; ChatGPT has so far failed to come up with its own questions when prompted to. Human needed to understand and generalise. I don't see any progress in LLMs towards that end.
...
Going forward, what's likely: we have tree of thought, where different nodes are evaluated by one model. We'll next have tree of models, where each node is its own model specialised on a task (potentially happening with racism detection in GPT4?), while each node's is evaluated by other models.
